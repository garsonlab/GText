		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				if (IN.texcoord1.y > 0)
				{
					...
					half index = abs(fmod(floor(_Time.x * _FrameSpeed * 50), IN.texcoord1.y));
					// compute real uv xy
					float x = fmod((IN.texcoord1.x+index), _LineCount)*_EmojiSize + IN.texcoord.x*_EmojiSize;
					float y = floor((IN.texcoord1.x+index)/_LineCount)*_EmojiSize + IN.texcoord.y*_EmojiSize;
